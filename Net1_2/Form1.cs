    ...
    public partial class Form1 : Form
    {
        ...
        private void ServerProc(object obj)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    form.Invoke(MessageHandle, $"Client {client.RemoteEndPoint} is connected");
        ...
        private void ClientProc(object obj)
        {
            ...
            try
            {
                ...
                par.form.Invoke(MessageHandle, $"Client: {par.name}");
                //3 comands
                while (true)
                {
                    ...
                    par.form.Invoke(MessageHandle, $"\'{par.name}\' sent command: \'{command}\'");
                    switch (command)
                    {
                        case "GetListClient": 
                            break;
            ...
            {
                par.form.Invoke(MessageHandle, $"User {par.name} disconnected!" );
                par.client.Close();
                //delete client from list
                //par.form.clientList.remove(par);
            }
        }
